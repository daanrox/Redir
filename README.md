...
## Descrição
Uma simples rota para fornecer scripts dinâmicos em sites.
Não é uma boa prática, mas para atender uma necessidade, implementamos essa API onde é possível injetar scripts dinâmicamente nas aplicações.

## Como Usar:

- Envie uma requisição GET para `/script` e receberá um script, variável ou informação desejada.
